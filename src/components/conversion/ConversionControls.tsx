...
import { generatePDF, generatePPTX } from "@/lib/ai-pdf-generation";
...
interface ConversionControlsProps {
  extractedContent?: {
    text: string;
    diagrams: Array<{ id: string; type: string; data: any; imageData?: string }>;
    sourceImage?: string;
  };
...
const ConversionControls = ({
  ...
  const generatePreviewSlides = (content: any, template: string) => {
    ...
    const slides = [
      {
        ...
        imageUrl: content.sourceImage || (content.diagrams && content.diagrams.length > 0 && content.diagrams[0].imageData) || "",
      },
    ];
    ...
    // Always include source image slide if setting is enabled
    if (settings.includeSourceImage) {
      slides.push({
        id: `slide-${slides.length + 1}`,
        title: "Original Whiteboard Image",
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const slidesData = generatedSlides.map((slide) => ({
        ...
        sourceImage: slide.title === "Original Whiteboard Image" || slide.id === "slide-1",
        originalImage: slide.title === "Original Whiteboard Image",
      }));
      
      // Create a copy of extractedContent with sourceImage if needed
      const processedContent = extractedContent ? {
        ...extractedContent,
        sourceImage: extractedContent.sourceImage || (extractedContent.diagrams?.[0]?.imageData ?? ""),
      } : null;
